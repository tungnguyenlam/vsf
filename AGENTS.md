...
`make help` lists the top-level targets. `make reproduce-pi` re-runs every prompt-injection number cited in `writeup/report.typ` and `writeup/report-vi.typ` (no LLM spend; reads on-disk data and the translation cache). `make test-pi` runs the test that pins the writeup tables to the reproducer. `make reproduce-pii` re-runs `regex_recall` on the 500-row val input_id manifest at `data/sample_ids/pii_masking_95k__validation__writeup_pin_500.json` (no NER model load; this is the cheapest deterministic slice of the PII report we can pin). `make test-pii` runs the PII pipeline tests (no model download); `make smoke-pii` runs `regex_only` on a 5-row HF sample to smoke-test the pipeline end-to-end. `make all` chains `reproduce-pi` + `test-pi` + `reproduce-pii` + `test-pii` + `smoke-pii`. Use these instead of hand-rolling commands.

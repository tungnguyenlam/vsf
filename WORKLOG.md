...
## 2026-06-25 — Wire review queues into the webdemo review tab
- `webdemo/safety_v0_review.py`: `list_canonical_files()` now surfaces `review/queue/*.jsonl` and lists them FIRST, so the Annotate tab defaults to a curated, priority-sorted queue instead of a full-source dump. Added `_count_needs_review()` so each queue's dropdown label shows live remaining count (`(N to review)`), discounting rows already `human_reviewed` via overrides.
- Overrides from a queue route to the same `human_overrides/<slug>.jsonl` the full source uses (`_layer_path_for` maps `review/queue/<slug>.jsonl` by stem == slug), so `apply_review_overrides.py` merges them regardless of which view was used. No new wiring needed for save/recompute/router routes — they take a file path and were already source-agnostic.
- `tests/test_safety_v0_review.py`: added `test_review_queue_listed_first_with_count` (queue surfaces first, shows count, count drops after a human_reviewed override).
- `webdemo/README.md`: documented the queue-first picker, the per-queue counts, and that queue overrides share the per-source override file.
- Verified end-to-end against real queues: all four surface (`mm_safetybench 22, unsafebench 344, vlguard 883, webpii 100`); unsafebench loads via `load_rows` with P1 PII-gap rows first, override file `human_overrides/unsafebench.jsonl`, `_label_mask` present.
- Verified: full suite 319 passed, 1 skipped.
- Residual risk: queue files are gitignored data, so they exist only on this machine until regenerated by `build_review_queue.py`. No code committed depends on their presence.

## 2026-06-25 — Clear the unsafebench 31 P1 pii_visible-gap review rows
- Added `scripts/safety_v0/clear_pii_gap_p1.py`: bulk-confirms the P1 review gap
  (rows with `pii_spans` detected but `pii_visible` not true). For each gap row it
  writes the same human-override `webdemo.safety_v0_review.save_override` produces
  when a reviewer toggles `pii_visible` and saves — sets ONLY `pii_visible=true`
  (other labels keep their weak/source_gold provenance, not re-stamped human) and
  marks `review.status=human_reviewed`. Idempotent: skips rows already reviewed.
  Flags: `--slug` (default unsafebench), `--input`, `--reviewer`, `--dry-run`.
- Ran on unsafebench: dry-run matched exactly the 31 P1 rows (first in the queue),
  then cleared all 31 -> `data/safety_v0/review/human_overrides/unsafebench.jsonl`.
  Verified via `load_rows`: reviewed 31, remaining 313 (was 344); cleared rows show
  `pii_visible=true` source=human, status=human_reviewed, `action` still source_gold;
  `_count_needs_review` dropped 344->313; re-run found 0 (idempotent).
- Tests: `tests/test_clear_pii_gap_p1.py` (3: gap predicate, clears-only-gap +
  provenance preserved, idempotent). safety_v0/review-queue suite 61 passed.
- Residual risk: cleared P1 rows also carried P3 (visual axes null) / P4
  (redaction_metadata) reasons; marking them human_reviewed drops them from the
  queue entirely, so those secondary axes were NOT separately confirmed — by
  design for this P1-only pass. Override file is gitignored data (local only).

## 2026-06-25 — Cap unsafebench queue at the P1+P2 set
- Reran `build_review_queue.py --slug unsafebench --limit 163` to bound the queue
  to P1(31)+P2(132); 181 P3-only rows (image rejected, visual axes null) dropped.
- The 31 P1 overrides survive the regen (stored in human_overrides/, keyed by

...
| `unsafebench` | [unsafebench.md](unsafebench.md) | safety_v0 source: DUA-gated 10,146-image visual-safety benchmark, 11 OpenAI content-policy categories (Hate, Harassment, Violence, Self-Harm, Sexual, Shocking, Illegal Activity, Deception, Political, Health, Spam), LAION-5B + Lexica. Download/inspect scripts committed + tested; data not pulled yet (no DUA approval). Sexual/Violence/Political mapped; blood_gore null (no upstream sub-label); other categories -> action=reject only. |
